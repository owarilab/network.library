...
// main
{
    ...
    else if( extension == 'js')
    {
        if( file_exist( file_path ) ){
            body = file_get( file_path );
            html = make_js_response( 200, body );
        }
        else{
            html = make_html_response( 404, "404 Not Found" );
        }
    ...
    else if( extension == 'css')
    {
        if( file_exist( file_path ) ){
            body = file_get( file_path );
            html = make_css_response( 200, body );
        }
        else{
            html = make_html_response( 404, "404 Not Found" );
        }
...
def make_file_response( file_path )
{
    ...
    date = gmtime();
    header = header_common( http_status_code, 'application/octet-stream' );
...
def make_png_response( file_path )
{
    ...
    header = header_common( http_status_code, 'image/png' );
...
def make_html_response( http_status_code, body )
{
    ...
    header = header_common( http_status_code, 'text/html' );
    header = header + "Date: "+date+"\r\n";
    header = header + "Content-Length: "+body_length+"\r\n";
    header = header + "\r\n";
	if( _HEADER['HTTP_METHOD']=='HEAD'){
...
def make_js_response( http_status_code, body )
{
    date = gmtime();
    body_length = count( body );
    header = header_common( http_status_code, 'text/javascript' );
    header = header + "Date: "+date+"\r\n";
    header = header + "Content-Length: "+body_length+"\r\n";
    header = header + "\r\n";
	if( _HEADER['HTTP_METHOD']=='HEAD'){
...
def make_css_response( http_status_code, body )
{
    date = gmtime();
    body_length = count( body );
    header = header_common( http_status_code, 'text/css' );
    header = header + "Date: "+date+"\r\n";
    header = header + "Content-Length: "+body_length+"\r\n";
    header = header + "\r\n";
	if( _HEADER['HTTP_METHOD']=='HEAD'){
...
def header_common( http_status_code, content_type )
{
    header = "";
