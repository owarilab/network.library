...
int gdt_http_protocol_filter(QS_RECV_INFO* rinfo)
{
	...
	if( psockparam->opcode == 2 ){
		...
		if( -1 != gdt_get_hash( option->memory_pool, psockparam->http_header_munit, "CONTENT_LENGTH" ) ){
			rinfo->recvlen = atoi( (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, psockparam->http_header_munit, "CONTENT_LENGTH" )) );
		}
...
int gdt_http_parser( QS_RECV_INFO *rinfo )
{
	...
	do{
		...
		if( QS_SYSTEM_ERROR == gdt_escape_directory_traversal( request_path, headerparam, sizeof(request_path) ) ){
	...
	do{
		...
		for(;;){
			if(psockparam->opcode==3){
				if( -1 != gdt_get_hash( option->memory_pool, psockparam->http_header_munit, "CONTENT_LENGTH" ) ){
					int contentlen = atoi( (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, psockparam->http_header_munit, "CONTENT_LENGTH" )) );
					size_t write_size = 0;
					size_t current = 0;
			...
			if( psockparam->opcode == 1 ){
				if(gdt_get_hash(option->memory_pool, psockparam->http_header_munit, "CONTENT_TYPE") != -1){
					char* contentType = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash(option->memory_pool, psockparam->http_header_munit, "CONTENT_TYPE"));
					char* pt = contentType;
					char param[256];
					char formkey[256];
...
int32_t gdt_http_parse_request_parameter(QS_MEMORY_POOL * memory,char *get_params)
{
	...
	do{
		...
		if( -1 == ( memid_get_parameter_hash = gdt_create_hash( memory, 32 ) ) ){
			break;
		}
		for(;*pparam != '\0';){
			...
			gdt_add_hash_value( memory, memid_get_parameter_hash, param_name, param_urldecode,ELEMENT_LITERAL_STR );
		}
	}while(false);
...
int32_t http_request_common(QS_RECV_INFO *rinfo, QS_HTTP_REQUEST_COMMON* http_request, QS_MEMORY_POOL* temporary_memory)
{
	...
	gdt_hash_dump(option->memory_pool, memid_headers,0);

	http_request->temporary_memory = temporary_memory;
	http_request->method = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, memid_headers, "HTTP_METHOD" ));
	http_request->request = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, memid_headers, "REQUEST" ));
	http_request->get_params = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, memid_headers, "GET_PARAMS" ));
	http_request->content_type = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, memid_headers, "CONTENT_TYPE" ));
	http_request->cache_control = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, memid_headers, "CACHE_CONTROL" ));
	http_request->http_version = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, memid_headers, "HTTP_VERSION" ));
	http_request->user_agent = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, memid_headers, "USER_AGENT" ));
	printf("method : %s , request : %s\n",http_request->method,http_request->request);

	int32_t memid_get_parameter_hash = -1;
	do{
		// get parameter
		if(-1==(memid_get_parameter_hash = gdt_http_parse_request_parameter(temporary_memory,http_request->get_params))){
			break;
		}
		printf("get params\n");
	...
	do{
		int32_t memid_request_path = gdt_create_memory_block(temporary_memory,MAXPATHLEN);
		if( -1 == memid_request_path ){
			break;
		}
		int32_t memid_extension = gdt_create_memory_block(temporary_memory,32);
		...
		gdt_http_document_path(http_request->request_path,MAXPATHLEN,"./www","index.html",http_request->request);

		http_request->extension = (char*)QS_GET_POINTER(temporary_memory,memid_extension);
		memset(http_request->extension,0,32);
		gdt_get_extension( http_request->extension, 32, http_request->request_path );
	}while(false);
	...
	if( !strcmp("POST",http_request->method) ){
		...
		do{
			if( !strcmp("application/json",http_request->content_type)){
				...
				if(-1==(memid_post_parameter_hash = gdt_http_parse_request_parameter(temporary_memory,body))){
					break;
				}
			}
		}while(false);
	...
	do{
		if( QS_SYSTEM_ERROR == gdt_fget_info( http_request->request_path, &http_request->file_info ) ){
			http_request->http_status_code = 404;
			break;
		}
		//response
		int32_t response_buffer_munit = gdt_create_memory_block(temporary_memory,http_request->file_info.size+SIZE_KBYTE);
		if( -1 == response_buffer_munit ){
			break;
		}
		char* response_buffer = (char*)QS_GET_POINTER(temporary_memory,response_buffer_munit);
		size_t response_buffer_size = gdt_usize(temporary_memory,response_buffer_munit);
		...
		if( !strcmp(http_request->extension,"html"))
		{
			...
			response_len = gdt_http_add_response_common(response_buffer,response_buffer_size,http_request->http_status_code,"text/html",http_request->file_info.size);
			response_len = gdt_http_add_cache_control(response_buffer, response_buffer_size, response_len, 30, &http_request->file_info);
		}
		else if( !strcmp(http_request->extension,"css"))
		{
			if( !strcmp(http_request->cache_control,"max-age=0") ){
				char *modified_since = (char*)QS_GET_POINTER(option->memory_pool,gdt_get_hash( option->memory_pool, memid_headers, "IF_MODIFIED_SINCE" ));
			...
			http_request->http_status_code = 200;
			response_len = gdt_http_add_response_common(response_buffer,response_buffer_size,http_request->http_status_code,"text/css",http_request->file_info.size);
			response_len = gdt_http_add_cache_control(response_buffer, response_buffer_size, response_len, 30, &http_request->file_info);
		}
		else if( !strcmp(http_request->extension,"js"))
		{
			...
			http_request->http_status_code = 200;
			response_len = gdt_http_add_response_common(response_buffer,response_buffer_size,http_request->http_status_code,"text/javascript",http_request->file_info.size);
			response_len = gdt_http_add_cache_control(response_buffer, response_buffer_size, response_len, 30, &http_request->file_info);
		}
		else if (!strcmp(http_request->extension, "json"))
		{
			...
			http_request->http_status_code = 200;
			response_len = gdt_http_add_response_common(response_buffer, response_buffer_size, http_request->http_status_code, "application/json", http_request->file_info.size);
			response_len = gdt_http_add_cache_control(response_buffer, response_buffer_size, response_len, 30, &http_request->file_info);
		}
		else if( !strcmp(http_request->extension,"ico"))
		{
			http_request->http_status_code = 200;
			response_len = gdt_http_add_response_common(response_buffer,response_buffer_size,http_request->http_status_code,"image/x-icon",http_request->file_info.size);
			is_binary = 1;
		}
		else if (!strcmp(http_request->extension, "unityweb"))
		{
			http_request->http_status_code = 200;
			response_len = gdt_http_add_response_common(response_buffer, response_buffer_size, http_request->http_status_code, "application/octet-stream", http_request->file_info.size);
			is_binary = 1;
		}
		else if (!strcmp(http_request->extension, "png"))
		{
			http_request->http_status_code = 200;
			response_len = gdt_http_add_response_common(response_buffer, response_buffer_size, http_request->http_status_code, "image/png", http_request->file_info.size);
			is_binary = 1;
		}
		else if (!strcmp(http_request->extension, "mp3"))
		{
			http_request->http_status_code = 200;
			response_len = gdt_http_add_response_common(response_buffer, response_buffer_size, http_request->http_status_code, "audio/mp3", http_request->file_info.size);
			is_binary = 1;
		}
		else{
			http_request->http_status_code = 404;
		...
		if( !(!strcmp("HEAD",http_request->method)) )
		{
			...
			if(is_binary==0){
				size_t readlen = gdt_fread_bin( http_request->request_path, pstart, plen );
